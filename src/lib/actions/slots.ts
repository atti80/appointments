...
export async function createSlotAction(formData: FormData): Promise<ActionResult> {
    ...
    if (!practitioner_id) return { success: false, error: 'Practitioner is required' }
...
export async function deleteSlotAction(slotId: string): Promise<ActionResult> {
    ...
    if (slot.status === 'pending') {
        return { success: false, error: 'Cannot delete a slot with a pending booking' }
    }
...
export async function applyTemplateAction(
    ...
    const proposed: { starts_at: string; ends_at: string; slot_type_id: string | null }[] = []
    ...
    const { data: conflicts } = await serviceClient.rpc('check_template_conflicts', {
...
export async function applyTemplateSkipConflictsAction(
    ...
    if (toInsert.length === 0) {
        return { success: false, error: 'All proposed slots conflict with existing ones' }
    }
